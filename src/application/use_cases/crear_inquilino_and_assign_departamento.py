...
class CreateInquilinoAndAssignDepartamento:
    ...
    def execute(self, command: CreateInquilinoCommand) -> int | None:
        ...
        print(f"Inquilino {command.nombre_completo} alojado en el departamento numero {departamento.num_departamento}")

        # marcar departamento como ocupado
        departamento.mark_as_occupied()

        print(f"Departamento numero {departamento.num_departamento} cambio a {departamento.status.value}")
